...
export default function BookPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // System Logic: Requirement 8 - Automatically Creates Contact & Booking
    setSubmitted(true);
  };

  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10 font-sans">
      {/* Visual Isolation: This page covers the global sidebar for the customer */}
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-50 overflow-y-auto py-10">
          ...
          {!submitted ? (
            <>
              <header className="mb-6 text-center sm:mb-8">
                <div className="inline-block px-3 py-1 rounded-full bg-sky-50 text-[10px] font-bold uppercase tracking-wider text-sky-700 mb-2 border border-sky-100">
                  No Account Required
                </div>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mt-2 text-sm text-slate-600">
                A confirmation link has been sent to <strong>{form.email}</strong>. Please check your inbox
                to finalize your appointment details and complete your intake forms.
              </p>
              <button 
                onClick={() => setSubmitted(false)}
                className="mt-6 text-xs text-slate-400 hover:text-sky-600 underline transition"
              >
